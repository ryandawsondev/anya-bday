...
// The date everything started — June 30, 2024
...
export default function EpilogueScreen({ onClose }: Props) {
  const daysTogether = Math.floor(
    (Date.now() - TOGETHER_SINCE.getTime()) / 86400000,
  )
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        inset: 0,
        background:
          'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(20,10,40,1) 0%, #000208 100%)',
        display: 'flex',
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            'radial-gradient(ellipse 50% 35% at 50% 50%, rgba(180,120,255,0.09) 0%, transparent 70%)',
          pointerEvents: 'none',
        }}
      />
      ...
      >
        ...
        <motion.div
          ...
          style={{
            height: '1px',
            background:
              'linear-gradient(90deg, transparent, rgba(180,150,255,0.25), transparent)',
            marginBottom: '2.5rem',
        ...
        {LETTER_LINES.map((line, i) => (
          <motion.p
            ...
            style={{
              ...
              fontFamily: 'Georgia, serif',
              fontSize: line.startsWith('Happy')
                ? 'clamp(1.35rem, 3.8vw, 1.9rem)'
                : 'clamp(0.95rem, 2.5vw, 1.1rem)',
              fontStyle: line.startsWith('Happy') ? 'italic' : 'normal',
              fontWeight: 400,
              color: line.startsWith('Happy')
                ? '#e8e4dc'
                : 'rgba(200,210,230,0.72)',
              letterSpacing: line.startsWith('Happy') ? '0.06em' : '0.03em',
              lineHeight: 1.75,
              textShadow: line.startsWith('Happy')
                ? '0 0 48px rgba(180,150,255,0.35)'
                : 'none',
            }}
          >
            {line || ' '}
        ...
        <motion.div
          ...
          transition={{
            duration: 0.8,
            delay: 1.9 + LETTER_LINES.length * 0.42 + 0.2,
            ease: 'easeOut',
          }}
          style={{
            height: '1px',
      ...
      <motion.button
        ...
        transition={{
          delay: 1.9 + LETTER_LINES.length * 0.42 + 1.0,
          duration: 0.5,
        }}
        onClick={onClose}
        style={{
